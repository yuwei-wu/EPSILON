...
namespace planning
{
  ...
  ErrorType SscPlanner::RunOnce()
  {
    ...
    // decide whether or noe the lateral velocity is independent
    ...
    } // for each behavior, build the corridor and ssc map
  ...
  if (ValidateTrajectory(*traj) != kSuccess) {
    ...
    eval_t3_ += (t_stf + t_sscmap + t_opt);
    eval_num3_ += 1;
    std::cout <<  " =============opt is "  << eval_t3_ << std::endl;
    std::cout <<  " =============eval_num3 is "  << eval_num3_ << std::endl;
